...
1. **Data Preprocessing & Balancing:** Addressed severe class imbalance within the educational dataset using **SMOTE** combined with rigorous feature preprocessing in Python (`Scikit-Learn`) to ensure robust training.
2. **Model Training:** Trained and evaluated a **Random Forest** classifier, achieving a high performance of **91.4% accuracy** and a **0.94 F1-score**.
3. **Feature Importance (Explainable AI):** Extracted granular feature importances from the model to pinpoint exact risk drivers:
